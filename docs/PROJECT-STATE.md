...
| Technical direction | Laravel/PHP with Livewire evaluated only; not selected or implemented. |
| Data store | PostgreSQL vs MySQL evaluation open. |
| External providers | Video, realtime, translation, payments, hosting, cloud, storage, messaging, and email/SMS providers open. |
| Key decisions | Medical/legal boundary, jurisdictions, verification, appointment policy, translation policy, and SEO analysis remain open. |
